...
// Alle Rezepte abrufen (ohne Filter)
                    ...
                    <?php foreach ($rezepte as $rezept): ?>
                        <div class="rezept-kachel">
                            <?php if (!empty($rezept['bild'])): ?>
                                <img src="data:image/jpeg;base64,<?= htmlspecialchars(base64_encode($rezept['bild'])); ?>"
                                    alt="<?= htmlspecialchars($rezept['titel']); ?>" class="recipe-image">
                            <?php else: ?>
                                <img src="../../assets/images/ImagePlaceholder.jpg" alt="<?= htmlspecialchars($rezept['titel']); ?>"
                                    class="recipe-image">
                            <?php endif; ?>
                            <h3>
                                <a
